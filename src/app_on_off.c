...
app_onoff_status_t app_onoff_status = {
    .onoff_status = ZCL_CMD_ONOFF_OFF,
};
...
void cmdOnOff(uint8_t command) {
    ...
    DEBUG(DEBUG_ONOFF_EN, "cmdOnOff() - cmd: %d\r\n", command);
...
#if NV_ENABLE
    app_onoff_status_t onoff_status;

    st = nv_flashReadNew(1, NV_MODULE_ZCL,  NV_ITEM_ZCL_ON_OFF, sizeof(app_onoff_status_t), (u8*)&onoff_status);

    if (st == NV_SUCC) {
        if(app_onoff_status.onoff_status != onoff_status.onoff_status) {
            onoff_status.onoff_status = app_onoff_status.onoff_status;
            st = nv_flashWriteNew(1, NV_MODULE_ZCL, NV_ITEM_ZCL_ON_OFF, sizeof(app_onoff_status_t), (u8*)&onoff_status);
        }
...
//    printf("zcl_onOffCfgAttr_restore()\r\n");
    ...
    if (st == NV_SUCC) {
        app_onoff_status.onoff_status = onoff_status.onoff_status;
    } else {
        app_onoff_status.onoff_status = ZCL_CMD_ONOFF_OFF;
    }
